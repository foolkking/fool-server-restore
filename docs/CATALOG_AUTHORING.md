...
- [ ] **每个 `package: name:` 里的包都填了 §7.5.2 的可用性矩阵**
- [ ] **凡矩阵里有 ⚠️ 或 ❌ 的包，已配 fallback（§7.5.3）**
- [ ] **fallback 脚本里的 GitHub release URL 已对照 `https://api.github.com/repos/<o>/<r>/releases/latest` 验过 asset 名称**
- [ ] **fallback 用了固定版本号，没用 `releases/latest/download/`**
...
## 七点五、包可用性审查（防 `No match for argument` 类故障）

> 这一节是基于真实生产事故沉淀的硬规矩：`exa → eza → lsd` 三连失败、PHP-FPM socket 路径漂移、`postgresql` 包名分裂等故障，本质都是"作者凭印象写包名，没在目标发行版上实际验证"。下面的检查表必须**每次新增 / 修改任何 `package` / `shell install` 任务时**逐条过。

### 7.5.1 凡是写进 `package: name:` 的包名，必须**当场**验证

不能凭社区博客 / GitHub README 的"安装一行"截图作数。每个包名都要回答清楚两个问题：

| 问题 | 验证命令（实跑） |
|---|---|
| Debian/Ubuntu 主流版本（22.04 / 24.04 / Debian 12）的官方仓库里**有**这个包吗？ | `apt-cache policy <pkg>` 或 https://packages.ubuntu.com/search?keywords=&searchon=names |
| RHEL 系（Anolis/Rocky/Alma 8/9）**默认仓库 + EPEL** 里有吗？ | `dnf info <pkg>`，或 https://pkgs.org/search/?q=<pkg> 选 "EL9" |
| 包名在两边**是否一致**？不一致需在 `PACKAGE_ALIASES` 里加映射 | 见 `apps/api/src/engine/modules/package.ts` |
| 包名是否曾经**改名 / 弃用**？（如 `exa` → `eza` → `lsd`） | 看上游 GitHub README 顶部、最近 6 个月的 release notes |

> ⚠️ **特别警惕**：EPEL 不是"全国统一发行版"。**国内厂商定制镜像**（阿里云 Anolis、华为 OpenEuler、腾讯 TencentOS、Aliyun Linux）经常**剔除部分 EPEL 包**或推自家定制版本。`/etc/dnf/dnf.conf` 里 `exclude=` 是常见手段。**不能假设"EPEL 有 → 用户机器上一定能装到"**。

### 7.5.2 包名审查矩阵（必须在 PR 描述贴出）

PR 提交前，给每个新引入的包做这张表：

```markdown
| 包名 | Ubuntu 22.04 | Ubuntu 24.04 | Debian 12 | RHEL 9 默认 | RHEL 9 + EPEL | 上游官方 deb/rpm/binary fallback |
|---|---|---|---|---|---|---|
| bat | ✅ (cmd: batcat) | ✅ (cmd: batcat) | ✅ (cmd: batcat) | ✅ | ✅ | github.com/sharkdp/bat |
| lsd | ✅ | ✅ | ✅ | ❌ | ⚠️ 部分镜像剔除 | github.com/lsd-rs/lsd（**仅 .deb + musl tarball，无 .rpm**） |
| eza | ❌ | ✅ | ✅（13+） | ❌ | ⚠️ 阿里云 Anolis 剔除 | github.com/eza-community/eza（仅 tarball） |
```

- 表里**任何一格 ❌ 或 ⚠️**，对应的 Playbook 都必须有 fallback 路径（见 7.5.3）
- 留空 / 写"应该有"是不可接受的
- 验证时要**至少在两台不同发行版的机器**（或 demo docker 容器）上跑一遍 `apt-cache search` / `dnf info`

### 7.5.3 fallback 策略（包名失败时该怎么办）

凡是上面矩阵里出现 ⚠️ 或 ❌ 的，Playbook 不能只写 `package: name: foo`，必须**双轨**：

```yaml
# 主轨：系统包管理器
- name: Install foo (system repo)
  module: package
  args:
    name: foo
    state: present
    ignore_missing: true     # 装不上不让整个 Playbook 失败

# 副轨：包不在仓库时从上游 release 安装
- name: Install foo (GitHub release fallback)
  module: shell
  args:
    cmd: |
      if command -v foo >/dev/null 2>&1; then
        echo "foo already installed: $(foo --version)"
        exit 0
      fi
      ARCH="$(uname -m)"
      VER="1.2.3"
      ...
      if command -v apt-get >/dev/null 2>&1; then
        URL="https://github.com/owner/foo/releases/download/v${VER}/foo_${VER}_${DEB_ARCH}.deb"
        curl -fsSLo /tmp/foo.deb "$URL"
        sudo dpkg -i /tmp/foo.deb && rm -f /tmp/foo.deb
      else
        # RHEL 系：上游不发 rpm 时走 musl 静态 tarball
        URL="https://github.com/owner/foo/releases/download/v${VER}/foo-v${VER}-${TAR_ARCH}-unknown-linux-musl.tar.gz"
        TMP="$(mktemp -d)"
        curl -fsSLo "$TMP/foo.tar.gz" "$URL"
        tar -xzf "$TMP/foo.tar.gz" -C "$TMP"
        sudo install -m 0755 "$TMP"/foo-*/foo /usr/local/bin/foo
        rm -rf "$TMP"
      fi
```

**关键点**：
1. **先 `command -v` 早退**，让重跑幂等。
2. **不要 `set -e`** 在 fallback 脚本顶部——它会让"主仓库装失败 → 走 fallback"这种合理流程被误判为 fail。
3. **校验上游 release 的真实 asset 名称**，别凭印象写。访问 `https://api.github.com/repos/<owner>/<repo>/releases/latest` 看 `assets[].name` 字段。
4. **静态 musl 二进制是 RHEL 通用兜底**——上游通常发 `<name>-<ver>-<arch>-unknown-linux-musl.tar.gz`，不依赖 glibc，绝大多数发行版都能跑。
5. **写明版本号**，不要用 `latest`（`https://github.com/.../releases/latest/download/...` 偶有 302 跳转问题，且无法回滚）。

### 7.5.4 上游不打 `.rpm` 的项目清单（已知）

许多 Rust / Go 项目**只发 `.deb` + tarball**，不打 RPM。RHEL 系遇到这些项目**只能走 tarball 路径**，不要尝试构造一个不存在的 rpm URL：

| 项目 | .deb | .rpm | 备注 |
|---|---|---|---|
| `lsd` | ✅ | ❌ | 必须用 musl tarball |
| `eza` | ❌ | ❌ | 仅 tarball |
| `bat` | ✅ | ❌ 上游 | `bat` 在 EPEL 有 |
| `fd` | ✅ | ❌ 上游 | `fd-find` 在 EPEL 有 |
| `ripgrep` | ✅ | ❌ 上游 | `ripgrep` 在 EPEL 有 |
| `zoxide` | ✅ | ❌ 上游 | `zoxide` 在 EPEL 有 |
| `micro` | ✅ | ❌ 上游 | `micro` 在 EPEL 有 |

> 如果你的项目不在这张表里，**先去查 GitHub release 实际 asset 列表再写**——不要假设。表格请保持更新（PR 时补行）。

### 7.5.5 失败两次必须停下来诊断根因

> 来自 [Kiro 工作守则](../README.md)：**同一方法连续失败两次，停下来分析根因，不做小补丁。**

`exa` 失败 → 改 `eza` 又失败 → 改 `lsd` 还失败，本质都是"猜包名"。**正确反应**：

1. 第二次失败时**停手**，不要立即改第三个包名。
2. 看错误日志的**真正信号**：`No match for argument` 说明仓库就是没这包，包名再换也白搭。
3. 用 `ssh user@host 'dnf search <关键词>'` 直接登目标机查，而不是去搜索引擎。
4. 确认根因是"包不存在"后，**走 fallback 路径**（7.5.3 的双轨）。

记录这次故障到本文档（"已知坑"段），下次别人不踩。

### 7.5.6 已知 catalog 包陷阱（持续更新）

| 包 / 场景 | 陷阱 | 应对 |
|---|---|---|
| `exa` | 2021 年起停止维护 | 一律换 `lsd`（不是 `eza`，因为 EPEL 镜像覆盖不全） |
| `eza` | 阿里云 Anolis 的 EPEL 镜像剔除 | 走 `lsd` 或上游 tarball |
| `tldr` | RHEL 系包名是 `tealdeer`，命令是 `tldr` | `PACKAGE_ALIASES` 已映射 |
| `bat` / `fd-find` | Ubuntu 命令是 `batcat` / `fdfind` | md 里说明 alias / symlink 方案 |
| `nginx` | Aliyun Anolis 默认 `exclude=nginx*`，推自家定制版 | 模块自动 `--disableexcludes=all` |
| `postgresql` | Debian 是 `postgresql`，RHEL 是 `postgresql-server` | `PACKAGE_ALIASES` 已映射 |
| `php-fpm` socket | Ubuntu `/run/php/php8.3-fpm.sock`，RHEL `/run/php-fpm/www.sock` | nginx 模板必须区分 |
| `redis-server` | Debian `redis-server`，RHEL `redis` | `PACKAGE_ALIASES` 已映射 |
| `default-jdk` | RHEL 没这个虚包 | 翻译为 `java-17-openjdk-devel` |
| 上游"latest" URL | GitHub `releases/latest/download/` 偶尔 302 异常 | 写定版号，不用 `latest` |
