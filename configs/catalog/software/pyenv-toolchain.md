...
Python 多版本管理工具，含编译依赖。

*Python multi-version manager with build deps.*

## 你将得到什么

- ▶ **安装 pyenv** _(Install pyenv)_ — 通过 curl script
...
```bash
# 检查包是否已安装
dpkg -l | grep git      # Ubuntu/Debian
rpm -q git                # RHEL/CentOS/Anolis

# 检查服务是否运行（如果有 systemd 单元）
systemctl status git --no-pager
```
