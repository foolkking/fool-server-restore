...
RabbitMQ 消息代理，支持 AMQP 协议，含管理面板。

*RabbitMQ message broker with AMQP support and management UI.*

## 你将得到什么

- 📦 **rabbitmq-server** _(RabbitMQ)_ — 通过 apt
...
```bash
# 检查包是否已安装
dpkg -l | grep rabbitmq-server      # Ubuntu/Debian
rpm -q rabbitmq-server                # RHEL/CentOS/Anolis
