...
现代化 Vim 分支，支持 Lua 配置和 LSP。

*Modern Vim fork with Lua config and LSP support.*

## 你将得到什么

- 📦 **neovim** _(Neovim)_ — 通过 apt
...
```bash
# 检查包是否已安装
dpkg -l | grep neovim      # Ubuntu/Debian
rpm -q neovim                # RHEL/CentOS/Anolis

# 检查服务是否运行（如果有 systemd 单元）
systemctl status neovim --no-pager
```
