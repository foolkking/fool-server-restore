...
nethogs、iftop、vnstat、tcpdump、nmap 等网络工具。

*nethogs, iftop, vnstat, tcpdump, nmap.*

## 你将得到什么

- 📦 **nethogs** _(nethogs)_ — 通过 apt
- 📦 **vnstat** _(vnstat)_ — 通过 apt
...
```bash
# 检查包是否已安装
dpkg -l | grep nethogs      # Ubuntu/Debian
rpm -q nethogs                # RHEL/CentOS/Anolis

# 检查服务是否运行（如果有 systemd 单元）
systemctl status nethogs --no-pager
```
