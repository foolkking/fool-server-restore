...
*Lightweight embedded SQL database, zero-config.*

## 你将得到什么

- 📦 **sqlite3** _(SQLite3)_ — 通过 apt
...
```bash
# 检查包是否已安装
dpkg -l | grep sqlite3      # Ubuntu/Debian
rpm -q sqlite3                # RHEL/CentOS/Anolis

# 检查服务是否运行（如果有 systemd 单元）
systemctl status sqlite3 --no-pager
```
