...
自托管文件同步和协作平台（snap 安装）。

*Self-hosted file sync and collaboration (via snap).*

## 你将得到什么

- ▶ **snap install nextcloud** _(snap install nextcloud)_ — 通过 snap
...
```bash
# 检查包是否已安装
dpkg -l | grep snapd      # Ubuntu/Debian
rpm -q snapd                # RHEL/CentOS/Anolis
