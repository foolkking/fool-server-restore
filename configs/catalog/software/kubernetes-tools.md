...
*kubectl + Helm orchestration tools.*

## 你将得到什么

- 📦 **kubectl** _(kubectl)_ — 通过 apt
- ▶ **Helm** _(Helm)_ — 通过 curl script
...
```bash
# 检查包是否已安装
dpkg -l | grep kubectl      # Ubuntu/Debian
rpm -q kubectl                # RHEL/CentOS/Anolis

# 检查服务是否运行（如果有 systemd 单元）
systemctl status kubectl --no-pager
```
