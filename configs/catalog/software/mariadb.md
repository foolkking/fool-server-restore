...
*Open-source MySQL-compatible database fork.*

## 你将得到什么

- 📦 **mariadb-server** _(MariaDB)_ — 通过 apt
...
```bash
# 检查包是否已安装
dpkg -l | grep mariadb-server      # Ubuntu/Debian
rpm -q mariadb-server                # RHEL/CentOS/Anolis

# 检查服务是否运行（如果有 systemd 单元）
systemctl status mariadb-server --no-pager
```
