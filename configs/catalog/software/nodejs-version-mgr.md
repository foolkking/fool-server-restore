...
Node.js 多版本管理工具，自动安装最新 LTS。

*Node.js multi-version manager with latest LTS.*

## 你将得到什么

- ▶ **安装 NVM** _(Install NVM)_ — 通过 curl script
...
2. Install NVM
3. Install latest LTS Node via NVM

## 验证安装

```bash
# 检查包是否已安装
dpkg -l | grep build-essential      # Ubuntu/Debian
rpm -q build-essential                # RHEL/CentOS/Anolis

# 检查服务是否运行（如果有 systemd 单元）
systemctl status build-essential --no-pager
```
